...
export default function Header({ onToggle }: { onToggle?: () => void }) {
  ...
  function mainSectionTitle(pathname: string): string {
    ...
    if (p.startsWith('/configuration-package') || p.startsWith('/client-test-suit')) return 'Configuration-package';
  ...
  return (
    <>
      {/* Disable Bootstrap’s default dropdown caret */}
      <style>{`
      ...
      <nav className="navbar navbar-top navbar-expand navbar-dark bg-dark-custom border-bottom">
        <div className="container-fluid">
          <div className="d-flex align-items-center">
            ...
            <button
              className="btn btn-link text-white p-0 mr-3 d-md-none"
              ...
              <li className={`nav-item dropdown ${open ? 'show' : ''}`} ref={toggleRef}>
                ...
                >
                  <div className="media align-items-center">
                    ...
                    {/* Custom caret icon — this one stays */}
                    <i
                      className={`caret-icon fa fa-caret-${open ? 'down' : 'right'} ml-2`}
                      aria-hidden="true"
                    />
                  </div>
                </button>
                ...
                <div className={`dropdown-menu dropdown-menu-right ${open ? 'show' : ''}`} style={{ position: 'absolute' }}>
                  <a className="dropdown-item" href="#" onClick={(e) => { e.preventDefault(); navigate('/profile/update'); }}>
                    <i className="ni ni-single-02" /> <span>My profile</span>
                  </a>
                  <a className="dropdown-item" href="#" onClick={(e) => { e.preventDefault(); navigate('/client-service-settings'); }}>
                  ...
                  <a className="dropdown-item" onClick={() => { unsetAuthUserData(); window.location.href = '/'; }}>
